...
class CounterTestCase(unittest.TestCase):
    ...
    def test_count_characters_mock_return_value(self):
        """Sprawdza, czy metoda count zwraca poprawnÄ… wartoÅ›Ä‡ dla danego argumentu."""
    ...
    def test_count_characters_mock_call_count_one(self):
        """Sprawdza, czy metoda count() zostala wywoÅ‚ana dokÅ‚adnie 1 raz."""
        mock_operation = Mock(spec=Operation)
        counter = Counter(mock_operation)

        counter.count_characters("Hello")

        self.assertEqual(mock_operation.count.call_count, 1)  # Powinno byc wywoÅ‚ane 1 raz
    ...
    def test_count_characters_mock_call_count_multiple(self):
        """Sprawdza, czy metoda count() zostaÅ‚a wywoÅ‚ana wielokrotnie."""
        mock_operation = Mock(spec=Operation)
        counter = Counter(mock_operation)
